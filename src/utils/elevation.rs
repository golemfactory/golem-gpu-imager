...
use windows_sys::Win32::{
    ...
    Security::{
        CheckTokenMembership, CreateWellKnownSid, GetTokenInformation, TOKEN_ELEVATION,
        TOKEN_QUERY, TokenElevation, WinBuiltinAdministratorsSid,
    },
...
pub fn get_elevation_status() -> String {
    #[cfg(windows)]
    {
        ...
        match (elevated, admin_user) {
            ...
            (false, true) => {
                "User is admin but process is not elevated. Use 'Run as administrator' to elevate."
                    .to_string()
            }
            (false, false) => {
                "User is not an administrator. Please log in as an administrator.".to_string()
            }
