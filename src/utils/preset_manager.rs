...
impl PresetManager {
    ...
    pub fn update_preset(&mut self, index: usize, preset: ConfigurationPreset) -> Result<(), String> {
    ...
    fn load_presets(&mut self) -> Result<(), String> {
        ...
        let mut file = File::open(&presets_path)
            .map_err(|e| format!("Failed to open presets file: {}", e))?;
        ...
        let presets_toml: PresetsToml = toml::from_str(&content)
            .map_err(|e| format!("Failed to parse presets TOML: {}", e))?;
