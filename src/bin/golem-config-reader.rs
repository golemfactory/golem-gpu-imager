...
impl Disk {
    ...
    fn dismount_volume(drive_path: &str) {
        ...
        if handle == INVALID_HANDLE_VALUE {
            ...
            warn!("Could not open volume for dismounting: error code {}", error_code);
            return;
        }
    ...
    fn check_disk_busy(path: &str) -> Result<bool> {
        use crate::win_constants::GENERIC_READ;
        use windows_sys::Win32::Foundation::{INVALID_HANDLE_VALUE, GetLastError, CloseHandle};
    ...
    fn get_drive_letter_for_disk(disk_num: usize) -> Option<String> {
        use windows_sys::Win32::Storage::FileSystem::{GetLogicalDrives, GetDriveTypeW};
    ...
    #[cfg(windows)]
    fn check_partition_style(handle: windows_sys::Win32::Foundation::HANDLE) -> Option<&'static str> {
        use windows_sys::Win32::System::IO::DeviceIoControl;
        use windows_sys::Win32::System::Ioctl::IOCTL_DISK_GET_DRIVE_LAYOUT;
    ...
    async fn open(path: &str) -> Result<Self> {
        ...
        let file = {
            ...
            if let Ok(disk_num) = path.parse::<usize>() {
                if let Some(drive_letter) = Self::get_drive_letter_for_disk(disk_num) {
                    info!("Found drive letter {} for physical disk {}", drive_letter, disk_num);
            ...
            info!("Attempting to open disk {} with several permission strategies", path_str);
            ...
            let access_modes = [
                ...
                (GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, "read access with full sharing"),
                // As a last resort, try with FILE_ATTRIBUTE_NORMAL flag
                (GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, "read access with normal attributes"),
            ];
            ...
            'access_mode_loop: for (access_rights, share_mode, description) in access_modes.iter() {
                for retry in 1..=MAX_RETRIES {
                    info!("Trying to open disk with {} (attempt {}/{})", description, retry, MAX_RETRIES);
                    ...
                    if handle != INVALID_HANDLE_VALUE {
                        ...
                        warn!("Failed to open with {}: error code {} (attempt {}/{})", 
            ...
            if handle == INVALID_HANDLE_VALUE {
                ...
                error!("Failed to open disk: error code {} - {}", last_error_code, error_msg);
                return Err(anyhow!("Failed to open disk: {} (error code {})", error_msg, last_error_code));
            }
        ...
        let file = match File::open(&path_str) {
            ...
            Err(e) => {
                ...
                return Err(anyhow!("Failed to open disk: {}. Make sure you have sufficient permissions.", e));
            }
        };
    ...
    fn find_partition<'a>(&'a mut self, uuid_str: &str) -> Result<fatfs::FileSystem<PartitionFileProxy<&'a File>>> {
        ...
        let disk = match disk_result {
            ...
            Err(e) => {
                ...
                #[cfg(windows)]
                {
                    ...
                    error!("For Windows users: This might be due to insufficient permissions or the disk being in use.");
                    error!("Try running as Administrator or using a different access method (PhysicalDrive# vs drive letter).");
                    ...
                    if result == 0 {
                        let error_code = unsafe { GetLastError() };
                        error!("Failed to get disk layout information. Error code: {}", error_code);
                    } else {
                        info!("Successfully retrieved disk layout information ({} bytes)", bytes_returned);
                ...
                #[cfg(not(windows))]
                {
                    ...
                    error!("For Linux users: This might be due to insufficient permissions. Try running with sudo.");
                }
                
                return Err(anyhow!("Failed to parse GPT partition table: {}. The disk may not have a valid GPT.", e));
            }
        };
        ...
        for (i, (_, part)) in partitions.iter().enumerate() {
            info!("Checking partition {}: UUID={}, Name={}", i, part.part_guid, part.name);
            ...
            if part.part_guid == target_uuid {
                ...
                match fs_result {
                    ...
                    Err(e) => {
                        ...
                        return Err(anyhow!("Found partition with matching UUID, but could not open as FAT filesystem: {}. The partition may be corrupt or not formatted as FAT.", e));
                    }
                }
            }
        }
        ...
        if !partitions.is_empty() {
            error!("No partition with UUID {} found. Available partitions:", target_uuid);
            for (i, (_, part)) in partitions.iter().enumerate() {
        ...
        Err(anyhow!("No partition found with UUID: {}. Make sure the disk contains a Golem configuration partition.", uuid_str))
    }
