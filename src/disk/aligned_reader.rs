...
impl<T: Read + Seek> AlignedReader<T> {
    ...
    pub fn new(inner: T, sector_size: usize, buffer_size: Option<usize>) -> Self {
        ...
        debug!("Creating AlignedReader with sector_size={}, buffer_size={}", 
    ...
    fn fill_buffer(&mut self) -> io::Result<usize> {
        ...
        if pos_offset > 0 {
            debug!("Aligning read: seeking from {} to {}", current_pos, aligned_pos);
        ...
        debug!("Buffer refilled: read {} bytes (available: {})", bytes_read, available);
        ...
        if self.buffer_pos >= self.buffer_len {
            ...
            match self.fill_buffer() {
                ...
                Ok(_) => {
                    ...
                    buf[copy_len..copy_len + new_copy].copy_from_slice(
                        &self.buffer[self.buffer_pos..self.buffer_pos + new_copy]
                    );
