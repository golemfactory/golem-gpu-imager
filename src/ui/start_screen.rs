...
pub fn view_start_screen<'a>(
    ...
    let error_container = if let Some(error) = error_message {
        ...
        .style(|_theme| container::Style {
            background: Some(if error.contains("Error") || error.contains("Failed") {
    ...
    let elevation_prompt = if !is_elevated && cfg!(windows) {
        let is_admin = crate::utils::is_admin_user();
        
        if is_admin {
            // User is admin but not elevated - show prominent elevation button
            container(
                column![
                    row![
            ...
            container(
                row![
                    icons::error().color(Color::from_rgb(0.8, 0.0, 0.0)),
                    text("Current user is not an administrator. Please log in as an administrator.")
                        .size(14)
