    ...
    task::sipper(async move |mut sipper| -> Result<MetadataProgress> {
        ...
        match result {
            ...
            Err(e) => {
                ...
                let progress = MetadataProgress::Failed { error: error_msg.clone() };
                sipper.send(progress.clone()).await;
...
fn calculate_metadata_blocking(
    ...
    loop {
        ...
        let estimated_progress = (total_uncompressed as f64 / estimated_uncompressed as f64).min(0.95) as f32;
