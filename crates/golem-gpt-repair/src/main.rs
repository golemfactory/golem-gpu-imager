...
#[clap(
    ...
    about = "Repair GPT headers with incorrect partition entry LBA values"
)]
struct Args {
...
fn main() -> Result<()> {
    ...
    if header_info.partition_entries_lba == args.target_lba {
        println!(
            "✓ GPT header already has the correct partition entries LBA value ({})",
            args.target_lba
        );
        return Ok(());
    }
    ...
    if args.dry_run {
        ...
        println!(
            "  ✗ Partition entries LBA is incorrect: {} (should be {})",
    ...
    if !args.yes {
        println!(
            "\n⚠️ WARNING: This operation will modify the GPT header on {}",
            args.device
        );
        println!(
            "Current partition entries LBA: {}",
            header_info.partition_entries_lba
        );
    ...
    println!(
        "  Partition entries LBA updated from {} to {}",
        header_info.partition_entries_lba, args.target_lba
    );
...
fn analyze_gpt_header(device_path: &str, verbose: bool) -> Result<GptHeaderInfo> {
    ...
    if header_bytes[0..8] != GPT_SIGNATURE {
        return Err(anyhow::anyhow!(
            "Invalid GPT signature - not a GPT disk or damaged header"
        ));
    }
    ...
    if verbose {
        ...
        println!(
            "  Partition Entry Array CRC32: {:08X}",
            partition_entry_array_crc32
        );
    }
    ...
    if current_lba != GPT_HEADER_SECTOR {
        println!(
            "⚠️ Warning: Current LBA ({}) is not the expected value for primary GPT header ({})",
    ...
    if partition_entries_lba == 2 {
        ...
        println!(
            "⚠️ Partition entries start at non-standard location: LBA {}",
            partition_entries_lba
        );
...
fn repair_gpt_header(
    device_path: &str,
    header_info: &GptHeaderInfo,
    target_lba: u64,
    verbose: bool,
) -> Result<()> {
    ...
    println!(
        "  Updated partition entries LBA from {} to {}",
        header_info.partition_entries_lba, target_lba
    );
    println!(
        "  Updated header CRC32 from {:08X} to {:08X}",
        header_info.header_crc32, new_crc32
    );
    ...
    if cfg!(windows) {
        #[cfg(windows)]
        {
            ...
            OpenOptions::new()
                ...
                .with_context(|| {
                    format!(
                        "Failed to open device for writing at path: {}",
                        path.display()
                    )
                })
        }
