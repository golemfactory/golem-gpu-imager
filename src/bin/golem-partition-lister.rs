...
fn dump_raw_sectors(device_path: &str) -> Result<()> {
    ...
    println!(
        "Read {} bytes from device ({} sectors)",
        bytes_read,
    ...
    if bytes_read >= SECTOR_SIZE {
        ...
        if check_mbr_signature(mbr_sector) {
            ...
            if partition_type == 0xEE {
                ...
                println!(
                    "✗ No protective MBR for GPT (partition type: {:02X})",
                    partition_type
                );
                ...
                for i in 0..4 {
                    ...
                    if type_code != 0 {
                        ...
                        println!(
                            "  Partition {}: Type {:02X}, Start Sector: {}, Sectors: {} ({}MB)",
                            i + 1,
                            type_code,
                            start_sector,
                            sector_count,
    ...
    if bytes_read >= SECTOR_SIZE * 2 {
        ...
        if check_gpt_signature(gpt_header_sector) {
            ...
            let revision = u32::from_le_bytes([
                gpt_header_sector[8],
                gpt_header_sector[9],
                gpt_header_sector[10],
                gpt_header_sector[11],
            ]);

            let header_size = u32::from_le_bytes([
                gpt_header_sector[12],
                gpt_header_sector[13],
                gpt_header_sector[14],
                gpt_header_sector[15],
            ]);

            let current_lba = u64::from_le_bytes([
                gpt_header_sector[24],
                gpt_header_sector[25],
                gpt_header_sector[26],
                gpt_header_sector[27],
                gpt_header_sector[28],
                gpt_header_sector[29],
                gpt_header_sector[30],
                gpt_header_sector[31],
            ]);

            let backup_lba = u64::from_le_bytes([
                gpt_header_sector[32],
                gpt_header_sector[33],
                gpt_header_sector[34],
                gpt_header_sector[35],
                gpt_header_sector[36],
                gpt_header_sector[37],
                gpt_header_sector[38],
                gpt_header_sector[39],
            ]);

            let first_usable_lba = u64::from_le_bytes([
                gpt_header_sector[40],
                gpt_header_sector[41],
                gpt_header_sector[42],
                gpt_header_sector[43],
                gpt_header_sector[44],
                gpt_header_sector[45],
                gpt_header_sector[46],
                gpt_header_sector[47],
            ]);

            let last_usable_lba = u64::from_le_bytes([
                gpt_header_sector[48],
                gpt_header_sector[49],
                gpt_header_sector[50],
                gpt_header_sector[51],
                gpt_header_sector[52],
                gpt_header_sector[53],
                gpt_header_sector[54],
                gpt_header_sector[55],
            ]);

            let partition_entries_lba = u64::from_le_bytes([
                gpt_header_sector[72],
                gpt_header_sector[73],
                gpt_header_sector[74],
                gpt_header_sector[75],
                gpt_header_sector[76],
                gpt_header_sector[77],
                gpt_header_sector[78],
                gpt_header_sector[79],
            ]);

            let num_partition_entries = u32::from_le_bytes([
                gpt_header_sector[80],
                gpt_header_sector[81],
                gpt_header_sector[82],
                gpt_header_sector[83],
            ]);

            let partition_entry_size = u32::from_le_bytes([
                gpt_header_sector[84],
                gpt_header_sector[85],
                gpt_header_sector[86],
                gpt_header_sector[87],
            ]);
            ...
            println!(
                "  Expected partition entries start at byte offset: {}",
            ...
            if partition_entries_lba == 2 {
                ...
                println!(
                    "! Partition entries start at non-standard location: LBA {}",
    ...
    if bytes_read >= SECTOR_SIZE * 3 {
        ...
        for entry_idx in 0..16 {
            // Look at first 16 potential entries
            ...
            if !is_empty {
                ...
                let size_mb =
                    ((last_lba - first_lba + 1) * SECTOR_SIZE as u64) as f64 / (1024.0 * 1024.0);
    ...
    if output.status.success() {
        ...
        if fdisk_output.status.success() {
            ...
            return Err(anyhow::anyhow!(
                "Both lsblk and fdisk failed to list partitions"
            ));
        }
    }
...
fn list_partitions(device_path: &str, verbose: bool) -> Result<Vec<PartitionInfo>> {
    ...
    let disk = match disk_result {
        ...
        Err(e) => {
            ...
            return Err(anyhow::anyhow!(
                "Failed to parse GPT partition table on device: {}",
                device_path
            )
            .context(e));
        }
    ...
    for (_i, (part_num, part)) in disk.partitions().iter().enumerate() {
        ...
        let size_bytes = if part.last_lba >= part.first_lba {
            ...
            debug!(
                "Warning: Invalid partition entry with last_lba < first_lba: {} < {}",
