...
use crate::ui::{
    ...
    edit_workflow::{EditState, EditWorkflowState},
    flash_workflow::{FlashMessage, FlashState, FlashWorkflowState},
    messages::Message,
    ...
    pub fn new() -> Self {
        ...
        let preset_manager_state = match &preset_manager_backend {
            Some(manager) => {
                ...
                // Select the default preset if available
                state.selected_preset = state.presets.iter().position(|p| p.is_default);
                state
            }
            None => {
                let mut state = PresetManagerState::with_defaults();
                // Select the default preset if available
                state.selected_preset = state.presets.iter().position(|p| p.is_default);
                state
            }
...
impl GolemGpuImager {
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::EditExistingDisk => {
                ...
                debug!(
                    "Entering edit existing disk mode - delegating device enumeration to DeviceSelection module"
                );
            ...
            Message::RefreshRepoData => self.load_repo_data(),
            ...
            Message::Edit(edit_msg) => {
                if let Some(edit_state) = &mut self.edit_workflow {
                    crate::ui::edit_workflow::handler::handle_message(
                        edit_state,
                        &self.device_selection,
                        edit_msg,
                    )
            ...
            Message::SelectPreset(index) => {
                // Apply preset configuration to current workflow
    ...
    pub fn view(&self) -> Element<Message> {
        match &self.mode {
            AppMode::StartScreen => crate::ui::start_screen::view_start_screen(
            ...
            async move {
                match repo.fetch_metadata().await {
                    Ok(metadata) => {
                        ...
                        let load_metadata_for_image =
                            |sha256: &str| -> Option<crate::models::ImageMetadata> {
                                if let Some(ref manager) = metadata_manager {
                                    ...
                                    match manager.load_metadata(sha256) {
                                        Ok(Some(metadata)) => {
                                            info!(
                                                "Successfully loaded metadata for SHA256: {} (uncompressed size: {} bytes)",
                                        ...
                                        Err(e) => {
                                            error!(
                                                "Failed to load metadata for SHA256 {}: {}",
                                                sha256, e
                                            );
                                            None
                                        }
                        ...
                        for channel in &metadata.channels {
                            ...
                            debug!(
                                "Channel '{}' latest version '{}': downloaded={}, path={}",
                                channel.name, latest_version.id, is_downloaded, latest_version.path
                            );
                            let image_path = if is_downloaded {
                                let path = repo
                                    .get_image_path(latest_version)
                                    .to_string_lossy()
                                    .to_string();
                                debug!(
                                    "Image path for downloaded version '{}': {}",
                                    latest_version.id, path
                                );
                                Some(path)
                            } else {
                                None
                            ...
                            let mut older_versions_sorted = channel
                                .versions
                                .iter()
                            ...
                            let older_versions: Vec<crate::ui::flash_workflow::OsImage> =
                                older_versions_sorted
                                    .iter()
                                    .map(|version| {
                                        let is_downloaded = repo.is_image_downloaded(version);
                                        let image_path = if is_downloaded {
                                            Some(
                                                repo.get_image_path(version)
                                                    .to_string_lossy()
                                                    .to_string(),
                                            )
                                        } else {
